...
# running ga-nginx::default as we need service[nginx] to be defined
describe 'ga-nginx::commons' do
  let(:chef_run) do
    ChefSpec::SoloRunner.new.converge('ga-nginx::default', described_recipe)
  end
  ...
  describe 'common recipes' do
    %w(
      ...
      it "includes the #{recipe} recipe" do
        expect(chef_run).to include_recipe("ga-nginx::#{recipe}")
      end
    end
  end
  ...
  describe 'common configurations' do
    ...
    context 'When the default website is disabled' do
      let(:chef_run) do
        ...
        end.converge('ga-nginx::default', described_recipe)
      end
